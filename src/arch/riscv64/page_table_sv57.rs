...
impl ArchPageTableEntryTrait for Riscv64PTEntry {
    ...
    fn entry(&self, index: usize) -> Self {
        ...
        unsafe {
            Riscv64PTEntry((addr as *const usize).read_volatile())
        }
    }
...
impl PageTable {
    ...
    pub fn map(&self, ipa: usize, pa: usize, pte: usize) {
        ...
        if l2e.valid() {
            ...
            unsafe { l1e.set_entry(pt_lvl2_idx(ipa), 
                Riscv64PTEntry::from_pte((pa >> 2) | pte)) };
        }
    }
    ...
    pub fn map_range(&self, ipa: usize, len: usize, pa: usize, pte: usize) {
        ...
        info!("map range ipa {:x} len {:x}, to pa {:x} pte {:x}. page_num = {}", ipa, len, pa, pte, page_num);
        for i in 0..page_num {
            self.map(ipa + i * PAGE_SIZE, pa + i * PAGE_SIZE, pte);
            if i % 0x10000 == 0 {
                info!("map ipa {:x} to pa {:x}", ipa + i * PAGE_SIZE, pa + i * PAGE_SIZE);
            }
        }
        info!("finish map_range!");
    }
