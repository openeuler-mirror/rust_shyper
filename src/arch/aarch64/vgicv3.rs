        ...
        if vgic_int_is_hw(interrupt.clone()) {
            ...
            if GICD.state(int_id) != 2 {
                if gic_is_priv(int_id) {
                    gic_set_state(int_id, 2, interrupt.phys_redist() as u32);
                } else {
                    gic_set_state(int_id, 2, 0);
                }
            }
        } else {
            ...
            if vgic_int_get_owner(vcpu.clone(), interrupt.clone()) {
                ...
                if interrupt.hw() {
                    if gic_is_priv(int_id) {
                        ...
                        gic_set_state(interrupt.id() as usize, if state == 1 { 2 } else { state }, 0);
                    }
                }
                self.route(vcpu.clone(), interrupt.clone());
                vgic_int_yield_owner(vcpu, interrupt.clone());
...
pub fn vgic_icc_sgir_handler(_emu_dev_id: usize, emu_ctx: &EmuContext) -> bool {
    if emu_ctx.write {
        ...
        if (sgir & GICC_SGIR_IRM_BIT) != 0 {
            targtlist = current_cpu().active_vcpu.clone().unwrap().vm().unwrap().ncpu() & !(1 << current_cpu().id);
        } else {
            ...
            if sgir & 0xff0000 != 0 {
                if cfg!(feature = "rk3588") {
                    //for rk3588 the aff1
                    vtarget = vtarget << ((sgir & 0xf0000) >> 16);
                }
            }
