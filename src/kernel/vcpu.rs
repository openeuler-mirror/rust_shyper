...
impl Vcpu {
    ...
    pub fn context_gic_irqs_store(&self) {
        ...
        let gicv_ctlr = unsafe { &*((Platform::GICV_BASE + 0x8_0000_0000) as *const u32) };
        inner.gic_ctx.set_gicv_ctlr(*gicv_ctlr);
        let gicv_pmr = unsafe { &*((Platform::GICV_BASE + 0x8_0000_0000 + 0x4) as *const u32) };
    ...
    pub fn context_gic_irqs_restore(&self) {
        ...
        let gicv_pmr = unsafe { &mut *((Platform::GICV_BASE + 0x8_0000_0000 + 0x4) as *mut u32) };
        *gicv_pmr = inner.gic_ctx.gicv_pmr();
        // println!("Core[{}] save gic context", current_cpu().id);
        let gicv_ctlr = unsafe { &mut *((Platform::GICV_BASE + 0x8_0000_0000) as *mut u32) };
...
pub fn show_vcpu_reg_context() {
    ...
    println!("GICV_CTLR {:x}", unsafe {
        *((Platform::GICV_BASE + 0x8_0000_0000) as *const u32)
    });
    println!("GICV_PMR {:x}", unsafe {
        *((Platform::GICV_BASE + 0x8_0000_0000 + 0x4) as *const u32)
    });
    println!("GICV_BPR {:x}", unsafe {
        *((Platform::GICV_BASE + 0x8_0000_0000 + 0x8) as *const u32)
    });
    println!("GICV_ABPR {:x}", unsafe {
        *((Platform::GICV_BASE + 0x8_0000_0000 + 0x1c) as *const u32)
    });
    println!("GICV_STATUSR {:x}", unsafe {
        *((Platform::GICV_BASE + 0x8_0000_0000 + 0x2c) as *const u32)
    });
    println!(
        "GICV_APR[0] {:x}, GICV_APR[1] {:x}, GICV_APR[2] {:x}, GICV_APR[3] {:x}",
        unsafe { *((Platform::GICV_BASE + 0x8_0000_0000 + 0xd0) as *const u32) },
        unsafe { *((Platform::GICV_BASE + 0x8_0000_0000 + 0xd4) as *const u32) },
        unsafe { *((Platform::GICV_BASE + 0x8_0000_0000 + 0xd8) as *const u32) },
        unsafe { *((Platform::GICV_BASE + 0x8_0000_0000 + 0xdc) as *const u32) },
    );
}
